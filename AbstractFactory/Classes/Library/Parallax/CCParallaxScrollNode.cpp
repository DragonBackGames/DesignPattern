...
void CCParallaxScrollNode::updateWithVelocity(CCPoint vel, float dt)
{
    ...
    for (int ii = 0; ii < _scrollOffsets->count(); ii++ )
    {
        CCParallaxScrollOffset *scrollOffset = (CCParallaxScrollOffset*)(_scrollOffsets->objectAtIndex(ii));
		CCPoint relVel = ccpMult(scrollOffset->getRelVelocity(), PTM_RATIO);
		CCPoint totalVel = ccpAdd(vel, relVel);
		CCPoint offset = ccpCompMult(ccp(totalVel.x * dt, totalVel.y * dt), scrollOffset->getRatio());
...
{
    CCArray *dealloc = CCArray::create();
    dealloc->autorelease();
...
void CCParallaxScrollNode::addInfiniteScrollWithObjects(CCArray *objects, int z, CCPoint ratio, CCPoint pos, CCPoint dir, CCPoint relVel, CCPoint padding)
{
    ...
    {
        CCSprite *object = (CCSprite*)(objects->objectAtIndex(ii));
